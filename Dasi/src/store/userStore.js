...
function createPersistedStore(key, startValue) {
    ...
    if (typeof window !== 'undefined') { // setter startValue til data hvis det finnes i localstorage eller så setter vi det til startValue
    ...
    if (typeof window !== 'undefined') { // kjører denne koden hvis vi er i en browser
        store.subscribe((value) => { // ser etter endringer i storen og setter den nye dataen (value) til localstorage
...
export const user = createPersistedStore('user', { // lager en store med key 'user' og startValue som er et objekt med id, name, email og picture som er null
    id: null,
    name: null,
    email: null,
    picture: null
