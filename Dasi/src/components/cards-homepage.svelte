<script>
    document.addEventListener('DOMContentLoaded', function () {
        var Card = document.querySelector('.card');
        var cardContent = document.querySelector('.card-content');
        
        if (Card) {
            Card.addEventListener('mouseenter', function () {
                cardContent?.classList.add('display-card');
            });
            Card.addEventListener('mouseleave', function () {
                cardContent?.classList.remove('display-card');
            });
        }
    });
</script>

<div class="card">
    <div class="card-content display-card">
        <h1>Climate-friendly</h1>
        <p>DASI-GPT is designed with sustainability in mind. By optimizing our data centers and using renewable energy sources, we significantly reduce our carbon footprint. Additionally, our efficient algorithms minimize energy consumption, making DASI-GPT a climate-friendly AI solution.</p>
        <p>Statistics show that our operations save approximately 1 million liters of water annually compared to traditional data centers. This is achieved through advanced cooling technologies and water recycling systems.</p>
    </div>
</div>


<style>
    .display-card {
        display: none;
    }

    .card {
        height: 30rem;
        display: flex;
        width: 20rem;
        background-image: url("../Images/cards-homepage/Climate-friendly.png");
        background-size: cover;
    }
</style>